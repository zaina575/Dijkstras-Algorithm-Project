...
#include <climits>
	...
	void detectCycle(int currIn, bool marked[], bool &isCycle);
	void cyclePrint(bool isCycle);
...
void dAlg(string input, int numberv, string varr[], bool marked[], int dist[], string prev[], Graph <string> &myGraph, Queue <string> &myQ){
	...
	detectCycle(currIn,marked,isCycle);
  ...
  void detectCycle(int currIn, bool marked[], bool &isCycle){
   ...
   void cyclePrint(bool isCycle){
	...
	if(isCycle == true){
		...
		cout <<  "\t" <<"\t" << "\t" << setw(40) << "The graph contains a cycle" << endl;
		}
	
	else {

		cout <<  "\t" <<"\t" << "\t" << setw(40) <<  "The graph does not contain a cycle" << endl;
	}
